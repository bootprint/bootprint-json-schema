...
describe('draft-04: types', function () {
  this.timeout(10000)
  ...
  it('should show simple types', function () {
    expect(bptest.textIn(select('simpleInteger', 'type', ''))).to.equal('Type (5.5.2) integer')
    expect(bptest.textIn(select('simpleString', 'type', ''))).to.equal('Type (5.5.2) string')
  })

  it('should show alternative types separated by "or"', function () {
    expect(bptest.textIn(select('stringOrInteger', 'type', ''))).to.equal('Type (5.5.2) string or integer')
  })
  ...
  it('should show an array as "array<*>" if no items-type is specified', function () {
    expect(bptest.textIn(select('simpleArray', 'type', ''))).to.equal('Type (5.5.2) array<*>')
  })

  it('should apply items-types to the array type ', function () {
    expect(bptest.textIn(select('stringOrStringArray', 'type', ''))).to.equal('Type (5.5.2) string or array<string>')
  })

  it('should show array types with multiple item-types', function () {
    expect(bptest.textIn(select('multiTypeArray', 'type', ''))).to.equal('Type (5.5.2) array<string|integer>')
  })
})
