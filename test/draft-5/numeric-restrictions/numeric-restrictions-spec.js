...
describe('draft-05: Numeric-restrictions', function () {
  this.timeout(10000)
  ...
  it('should display a the doclink to the draft-05-schema', function () {
    expect(bptest.$(select('age', 'minimum', '.header a:first-child')).attr('href'))
      .to.equal('https://tools.ietf.org/html/draft-wright-json-schema-validation-00#section-5.4')
  })

  it('should display the correct explanation text', function () {
    expect(bptest.textIn(select('age', 'minimum', '> .header')))
      .to.equal('Numeric range (5.4, 5.2, 5.5, 5.3)')
  })
  ...
  it('should display a the doclink to the draft-05-schema (multipleOf)', function () {
    expect(bptest.$(select('multiples_of_three', 'multipleOf', '.header a')).attr('href'))
      .to.equal('https://tools.ietf.org/html/draft-wright-json-schema-validation-00#section-5.1')
  })
