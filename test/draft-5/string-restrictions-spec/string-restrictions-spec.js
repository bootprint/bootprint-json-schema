...
describe('draft-05: string-restrictions', function () {
  this.timeout(10000)
  ...
  describe('the string-length section', function () {
    it('should display a the doclink to the draft-05-schema for minLength', function () {
      expect(bptest.$(select('string-min-length', 'minLength', '> .header a:first-child')).attr('href'))
        .to.equal('https://tools.ietf.org/html/draft-wright-json-schema-validation-00#section-5.7')
    })

    it('should display a the doclink to the draft-05-schema for maxLength', function () {
      expect(bptest.$(select('string-min-length', 'minLength', '> .header a:last-child')).attr('href'))
        .to.equal('https://tools.ietf.org/html/draft-wright-json-schema-validation-00#section-5.6')
    })

    it('should display the correct explanation text', function () {
      expect(bptest.textIn(select('string-min-length', 'minLength', '> .header')))
        .to.equal('String length (5.7, 5.6)')
    })
  ...
  describe('the pattern section', function () {
    it('should display a the doclink to the draft-05-schema for minLength', function () {
      expect(bptest.$(select('only-a', 'pattern', '> .header a')).attr('href'))
        .to.equal('https://tools.ietf.org/html/draft-wright-json-schema-validation-00#section-5.8')
    })

    it('should display the correct explanation text', function () {
      expect(bptest.textIn(select('only-a', 'pattern', '> .header')))
        .to.equal('String Pattern (5.8)')
    })
