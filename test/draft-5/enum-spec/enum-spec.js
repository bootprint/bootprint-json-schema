...
describe('draft-05: enum', function () {
  this.timeout(10000)
  ...
  it('should display the doclink to the draft-05-schema', function () {
    expect(bptest.$(select('mixed', 'enum', '.header a')).attr('href'))
      .to.equal('https://tools.ietf.org/html/draft-wright-json-schema-validation-00#section-5.20')
  })

  it('should display the correct explanation text', function () {
    expect(bptest.textIn(select('mixed', 'enum', '> .header')))
      .to.equal('Enum (allowed values) (5.20)')
  })
